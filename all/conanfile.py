...
class CorradeConan(ConanFile):
    ...
    topics = ("conan", "corrade", "magnum", "filesystem",
              "console", "environment", "os")
    ...
    def configure(self):
        if self.settings.compiler == "Visual Studio" and tools.Version(self.settings.compiler.version) < 14:
            raise ConanInvalidConfiguration(
                "Corrade requires Visual Studio version 14 or greater")

    def source(self):
        git = tools.Git(folder=self._source_subfolder)
        git.clone("https://github.com/mosra/corrade.git", "master")
    ...
    def package(self):
        ...
        share_cmake = os.path.join(
            self.package_folder, "share", "cmake", "Corrade")
        self.copy("UseCorrade.cmake", src=share_cmake, dst=os.path.join(
            self.package_folder, "lib", "cmake", "Corrade"))
        self.copy("CorradeLibSuffix.cmake", src=share_cmake, dst=os.path.join(
            self.package_folder, "lib", "cmake", "Corrade"))
    ...
    def package_info(self):
        ...
        self.cpp_info.build_modules.append(os.path.join(
            "lib", "cmake", "Corrade", "UseCorrade.cmake"))
        self.cpp_info.build_modules.append(os.path.join(
            "lib", "cmake", "Corrade", "CorradeLibSuffix.cmake"))
        ...
        self.cpp_info.builddirs = [os.path.join(
            self.package_folder, "lib", "cmake", "Corrade")]
...
#        if self.settings.os == "Emscripten" and self.settings_build.os == "Linux":
